...
## 16. 对于因为叠加元素导致的排版和错位问题，优先考虑“做减法” (Subtract before Over-Engineering)
- **踩坑点**：为了在全局统一注入复杂的上传按钮，导致 `#PopupBox` (纯文本快捷回复框) 里面出现了高度错位、文字溢出等 CSS 问题。之前的思路是继续堆砌更多的 CSS hack (如 `display: flex`, 精准计算 `height` 和 `box-sizing`) 去强行对齐。
- **复盘与规则**：
  - 当发现自己在一个原本非常简单的原生 DOM 上，为了实现某个注入功能而写出极其复杂的 CSS/JS 对齐修补代码时，**立刻停下来，思考“这个注入元素是否真的不可或缺”**。
  - 直接去掉多余的注入元素，借助原生既有类的力量 (比如完全复印 `#phupdate` 的简明样式) 来保持纯净，永远是最简单、Bug率最低的优雅解法。
